...
@pony.orm.db_session
def set_player_position(game_name,n_players):
    player_list = get_player_list(game_name)
    for i in range(n_players):
        player_list[i].position = i
...
def get_next_player_to_min(game_name):
    ...
    t = get_turn(t_id)
    player_list = get_player_list(game_name)
    player_list.sort(key=lambda p: p.position)
    if (t.imperius_minister_old == None) and (t.imperius_minister_new == None):
        last_min = t.previous_min
        if last_min is None:
            return player_list[0].id
        else:
            last_position = Player[last_min].position
            n = len(player_list)
            for p in range(n):
                if player_list[(last_position + p + 1) % n].is_alive:
                    return player_list[(last_position + p + 1) % n].id
    else:
...
@pony.orm.db_session
def new_finished_game(game_name, loyalty_win, reason):
    game = get_game_by_name(game_name)
    finish_game = FinishedGames(game_name=game_name, initial_date=game.initial_date,
                                end_date=game.end_date, win=loyalty_win, reason=reason)
    for p in game.players:
